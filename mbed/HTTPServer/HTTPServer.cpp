...
C12832_LCD lcd;
    ...
    lcd.cls();
    lcd.locate(0,0);
    lcd.printf("Start method of http server");
    ...
    lcd.cls();
    lcd.locate(0,0);
    lcd.printf("Created new EthernetInterface.");
    ...
    lcd.cls();
    lcd.locate(0,0);
    lcd.printf("Failed to initialize the ethernet interface.");
            delete m_pEthernet;
            m_pEthernet = NULL;
            return false;
        }
        ...
        INFO("Connecting to the network using DHCP...");
            lcd.cls();
    lcd.locate(0,0);
    lcd.printf("Connecting to network using DHCP....");
        if (m_pEthernet->connect() != 0) {
            lcd.cls();
    lcd.locate(0,0);
    lcd.printf("Failed to connect to the ethernet.");
            ...
            lcd.cls();
    lcd.locate(0,0);
    lcd.printf("Connected IP %s",m_pEthernet->getIPAddress());
        ...
        lcd.cls();
        lcd.locate(0,0);
    lcd.printf("Using IP %s", pEthernet->getIPAddress());
    ...
    lcd.cls();
        lcd.locate(0,0);
    lcd.printf("Failed to bind port ! %s",port);
...
lcd.cls();
        lcd.locate(0,0);
    lcd.printf("msg: %s",msg.uri.c_str());
